    ...
    <div class="w-full max-w-xl bg-white p-6 sm:p-8 md:rounded-[2.5rem] form-container mobile-card" style="box-shadow: var(--chaya-lg);">
        ...
        <div class="flex items-center justify-between gap-4 mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h2 class="text-2xl font-black text-slate-800 tracking-tight">Edit Earning / Income</h2>
                    <p class="text-xs font-semibold text-slate-400">Update details for this earning entry</p>
                </div>
            </div>
        ...
        <form method="POST" action="{{ route('incomes.update', $income->id) }}" class="space-y-5">
            ...
            <div class="bg-gradient-to-br from-emerald-50 via-teal-50/50 to-emerald-50/30 p-6 rounded-3xl border border-emerald-100">
                <label class="block text-xs font-black text-emerald-600 uppercase tracking-widest mb-1">Inflow Amount</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="space-y-1">
                    ...
                    <input type="text" name="source" placeholder="e.g. Sold Old Bike, Client X, Salary" required
                ...
                <div class="space-y-1">
                    ...
                    <select name="income_category_id" class="w-full bg-slate-50 border border-slate-100 p-4 rounded-2xl custom-input appearance-none text-slate-800 font-semibold cursor-pointer">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div class="space-y-1">
                    ...
                    <input type="date" name="income_date" value="{{ old('income_date', $income->income_date ? $income->income_date->format('Y-m-d') : date('Y-m-d')) }}"
                           class="w-full bg-slate-50 border border-slate-100 p-4 rounded-2xl custom-input text-slate-800 font-semibold">
                </div>
            </div>
            ...
            <div class="space-y-1">
                ...
                <input type="text" name="transaction_id" placeholder="e.g. UPI Ref / Bank UTR / Invoice #"
                       value="{{ old('transaction_id', $income->transaction_id) }}"
            ...
            <div class="space-y-1">
                ...
                <textarea name="note" rows="2" placeholder="Notes about this income..."
                          class="w-full bg-slate-50 border border-slate-100 p-4 rounded-2xl custom-input text-slate-800">{{ old('note', $income->note) }}</textarea>
            </div>

            <div class="flex items-center gap-3 pt-2">
                <button type="submit"
                        class="btn-emerald flex-1 bg-emerald-600 text-white p-4 rounded-2xl font-bold text-base shadow-xl shadow-emerald-200 hover:bg-emerald-700 flex justify-center items-center gap-2 min-h-[52px] transition cursor-pointer">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                    Save Changes
                </button>
                <a href="{{ route('incomes.index') }}" class="px-5 py-4 bg-slate-100 text-slate-600 rounded-2xl font-bold text-base hover:bg-slate-200 transition min-h-[52px] flex items-center justify-center">
                    Cancel
                </a>
            </div>
        </form>
